...
export default function MyStats() {
  ...
  const totalSpent = transactions
    .filter((t) => ["SEND", "CASH_OUT", "CASH_IN"].includes(t.type))
  ...
  const lineData = transactions.map((item) => ({
  ...
  const barData = Object.entries(typeSummary).map(([type, amount]) => ({ type, amount }));
  ...
  return (
    <TourWrapper steps={steps} tourId="user-stats-tour" autoStart={true}>
      <div className="space-y-4 sm:space-y-6 p-3 sm:p-4 md:p-6 max-w-full overflow-hidden">
        ...
        {/* Stats Cards */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4" data-tour="userStats1">
          <StatsCard
            title="Total Spent"
        ...
        <div className="space-y-4 sm:space-y-6">
          {/* Line & Bar charts */}
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 sm:gap-6" data-tour="userStats2">
              ...
              <CardContent className="h-48 sm:h-64 md:h-72 p-2 sm:p-6">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={lineData} margin={{ top: 5, right: 10, left: 5, bottom: 5 }}>
                    ...
                    <XAxis dataKey="date" fontSize={10} className="sm:text-xs" interval="preserveStartEnd" />
                    <YAxis fontSize={10} className="sm:text-xs" width={40} />
                    <Tooltip
                      contentStyle={{
                        fontSize: "12px",
                    ...
                    />
                    <Line type="monotone" dataKey="amount" stroke="#2563eb" strokeWidth={2} dot={{ r: 3 }} activeDot={{ r: 4 }} />
                  </LineChart>
                </ResponsiveContainer>
              </CardContent>
            </Card>
              ...
              <CardContent className="h-48 sm:h-64 md:h-72 p-2 sm:p-6">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={barData} margin={{ top: 5, right: 10, left: 5, bottom: 5 }}>
                    <CartesianGrid strokeDasharray="3 3" className="opacity-30" />
                    <XAxis dataKey="type" fontSize={10} className="sm:text-xs" angle={-45} textAnchor="end" height={50} />
                    <YAxis fontSize={10} className="sm:text-xs" width={40} />
                    <Tooltip
                      contentStyle={{
                        fontSize: "12px",
            ...
            <CardContent className="h-64 sm:h-80 md:h-96 p-2 sm:p-6">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  ...
                  />
                  <Legend wrapperStyle={{ fontSize: "12px" }} layout="horizontal" align="center" verticalAlign="bottom" />
                </PieChart>
              </ResponsiveContainer>
            </CardContent>
          </Card>
